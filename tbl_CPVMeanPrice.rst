...
Данная аналитическая таблица хранит в себе средние цены за единицы измерения часто закупаемых предметов закупки.
...
================= ======================= ======
Предмет закупки   Код единицы измерения   Цена
----------------- ----------------------- ------
Предмет закупки 1 Код единицы измерения 1 Цена 1     
Предмет закупки 1 Код единицы измерения 2 Цена 2
Предмет закупки 1 Код единицы измерения 3 Цена 3
Предмет закупки 2 Код единицы измерения 1 Цена 1
Предмет закупки 2 Код единицы измерения 2 Цена 2
....              ....                    ....
================= ======================= ======
...
3. Выбираем все процедуры, проведенные одноэтапным методом, упрощенным методом или методом на понижение, в статусе ``contractSigned``, процедуры, которые больше 30 дней находятся в статусе ``bidsOpened``.
4. Из каждой найденной процедуры извлекаем идентификаторы предметов закупки (конкатенация ``data.tender.items.classification.scheme`` и ``data.tender.items.classification.id``) и единицы измерения ``data.items.unit.id``.
...
6. Группируем полученные данные по коду предмета закупки и единице измерения, выбирая среднее по цене выигрышного предложения.
