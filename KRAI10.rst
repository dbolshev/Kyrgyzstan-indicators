...
6. Каждый элемент ``data.tender.items.classification.id`` из исследуемой процедуры должен находиться в найденной процедуре. Иначе, индикатор принимает значение ``1``. Расчет заканчивается.
