...
2. Для каждой найденной пары ``data.tender.items.classification.id`` и ``data.items.unit.id`` проводим следующие действия:
    ...
    - Если в аналитической таблице для данного кода предмета закупки отсутствует строка по стоимости единицы измерения, индикатор принимает значение ``0`` для ``data.lots.id = data.item.relatedLot``, расчет заканчивается.  
    - Если в аналитической таблице для данного кода присутствует строка со средним значением, вычисляем сколько процентов составляет цена из процедуры от средней цены в аналитической таблице.
