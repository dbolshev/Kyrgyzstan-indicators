...
***********************
1. Выбираем только процедуры проводимые одноэтапным методом, упрощенным методом или методом на понижение цены, у которых ``data.tender.status = 'active'`` или ``data.tender.status = 'complete'``. Не берем процедуры с плохим качеством данных.
