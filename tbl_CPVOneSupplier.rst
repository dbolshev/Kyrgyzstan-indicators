...
Данная аналитическая таблица хранит все коды предметов закупок для каждой закупающей организации и поставщика, которые поставщик поставлял закупающей организации по прямому договору по причине "приобретения товаров, работ и услуг по каждой статье расходов один раз в год до минимальной пороговой суммы". 
...
2. Выбираем только те процедуры, у которых ``data.tender.procurementMethodRationale = 'PROC_ONCE_IN_A_YEAR'``. И только процедуры на поставку товаров.
