...
8. В найденных выигравших предложения из предыдущего шага сравниваем суммы предложений ``data.bids.relatedLots.value.amount``. Если сумма исследуемой процедуры составляет больше 15% от суммы предшествующей конкурентной процедуры, индикатор принимает значение ``1``. Расчет заканчивается.
